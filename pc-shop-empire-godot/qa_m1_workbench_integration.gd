...
func run()->void:
	...
	check(bool(ui.install_component("cpu_9600x").get("ok",false)),"workbench installs cpu_9600x")
	check(bool(ui.install_component("ram_32").get("ok",false)),"workbench installs ram_32")
	check(not bool(ui.remove_component("Motherboard").get("ok",true)),"dependency rule blocks motherboard removal while CPU and RAM are installed")
	var previous_ram_count:=int(ui.core.inventory.get("ram_32",0))
	check(bool(ui.remove_component("RAM").get("ok",false)),"installed RAM can be removed from the real workbench UI")
	check(int(ui.core.inventory.get("ram_32",0))==previous_ram_count+1,"removed RAM returns to inventory")
	check(bool(ui.install_component("ram_32").get("ok",false)),"removed RAM can be reinstalled")
	check(bool(ui.install_component("ram_64").get("ok",false)),"compatible RAM replacement installs through the workbench")
	check(String(ui.core.slots.get("RAM",""))=="ram_64" and int(ui.core.inventory.get("ram_32",0))>=1,"replacement returns the previous RAM to inventory")
	for id:String in ["ssd_1tb","psu_750","gpu_5060"]: check(bool(ui.install_component(id).get("ok",false)),"workbench installs "+id)
