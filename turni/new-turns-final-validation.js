(()=>{
 ...
 function showChecking(){
  ...
  if(box){box.className='statuschecking';box.textContent='⏳ Controllo effettivo della turnazione in corso…'}
  const actions=main.querySelector('.actions');if(actions)actions.innerHTML='<button type="button" disabled class="validation-wait">Attendi il controllo finale…</button>';
 }
 function persistFinal(){
  try{const issues=validateWeek(draft);localStorage.setItem('tm_planner_draft',JSON.stringify({week:draft,issues,createdAt:new Date().toISOString()}));return issues}catch{return[]}
 }
 function finalize(){
  if(!draft)return;
 ...
 draw=function(used){
  ...
  if(ready){ready=false;previousDraw(used);showChecking()}else{previousDraw(used);showChecking()}
  clearTimeout(timer);timer=setTimeout(finalize,220);
 };
