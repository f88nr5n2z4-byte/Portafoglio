...
const ALLOWED=/^(?:16:(?:00|30)-(?:19:30|20:00|20:30)|08:00-13:00|RIPOSO)$/;
...
function marcoShift(d){const day=d.getDay();if(day===2)return'RIPOSO';if(day===1||day===3||day===4)return'16:00-20:00';const wk=isoWeek(d);if(day===5)return wk%2===1?'16:00-20:00':'RIPOSO';if(day===6)return wk%2===0?'16:00-20:00':'RIPOSO';return null}
...
function keepExisting(base,label){for(const x of base){const p=parsed(x);if(!p||p.label!==label)continue;if(PROTECTED.has(p.wanted))return true;if(p.id.startsWith('override-')&&ALLOWED.test(p.wanted))return true}return false}
function synthetic(base){const center=stateMonth(),from=new Date(center.getFullYear(),center.getMonth()-2,1,12),to=new Date(center.getFullYear(),center.getMonth()+3,0,12),out=[];for(let d=new Date(from);d<=to;d.setDate(d.getDate()+1)){const wanted=marcoShift(d);if(wanted===null)continue;const date=iso(d),label=fmt(date);if(keepExisting(base,label))continue;out.push({id:`marco-rule-${date}`,employee:'Marco',status:'ACCETTATA',replied_at:new Date().toISOString(),message:`Giorno: ${label}\nTurno richiesto: ${wanted}\nOrigine: disponibilità Marco`})}return out}
