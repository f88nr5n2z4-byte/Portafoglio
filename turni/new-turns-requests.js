(()=>{
 ...
 function persistAndRedraw(){
  if(!draft)return;
  applyStored(draft);
  const issues=validateWeek(draft);
  try{localStorage.setItem('tm_planner_draft',JSON.stringify({week:draft,issues,createdAt:new Date().toISOString()}))}catch{}
  originalDraw([]);
  mount();
 }
 function recalcAfterRequest(){
  if(!draft)return;
  applyStored(draft);
  originalRepair(draft);
  applyStored(draft);
  // Validazione fresca DOPO la richiesta e DOPO gli eventuali aggiustamenti automatici.
  validateWeek(draft);
  persistAndRedraw();
 }
 function mount(){
  ...
  card.querySelectorAll('[data-apply-req]').forEach(b=>b.onclick=()=>{
   const id=Number(b.dataset.applyReq),ids=new Set(loadApplied());ids.add(id);saveApplied([...ids]);
   const x=rel.find(y=>Number(y.r.id)===id);if(x){draft.schedule[x.r.employee][x.di]=x.wanted;locked.add(`${x.r.employee}|${x.di}`)}
   recalcAfterRequest();
  });
  card.querySelectorAll('[data-remove-req]').forEach(b=>b.onclick=()=>{
   const id=Number(b.dataset.removeReq),ids=new Set(loadApplied());ids.delete(id);saveApplied([...ids]);
   const prev=official?.weeks?.at(-1);
   if(prev){locked.clear();draft=buildBase(prev);applyAccepted(draft);if(typeof applyManual==='function')try{applyManual(draft)}catch{}applyStored(draft);originalRepair(draft);applyStored(draft);persistAndRedraw()}else persistAndRedraw();
  });
 }
 ...
 setTimeout(()=>{try{if(draft){applyStored(draft);originalRepair(draft);applyStored(draft);persistAndRedraw()}}catch(e){console.warn('planner requests init',e)}},120);
})();