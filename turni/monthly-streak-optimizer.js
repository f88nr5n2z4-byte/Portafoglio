(()=>{
'use strict';
const KEY='tm_monthly_streak_optimizer_v75';
...
function requests(base){const occupied=new Set((base||[]).filter(x=>!String(x.id||'').startsWith('streak-v75-')).map(acceptedCell).filter(Boolean));return load().filter(x=>!occupied.has(x.employee+'|'+fmt(x.date))).map(x=>({id:'streak-v75-'+x.employee+'-'+x.date,employee:x.employee,status:'ACCETTATA',replied_at:new Date().toISOString(),message:`Giorno: ${fmt(x.date)}\nTurno richiesto: RIPOSO\nOrigine: correzione automatica massimo 6 giorni consecutivi`}))}
...
async function optimize(){if(window.__tmFatal)return;const w=draft();if(!w?.dates?.length||!w.schedule)return;let base=[],official={weeks:[]};try{const r=await prevApi('list_requests');base=r.requests||[]}catch{}try{const r=await prevApi('get_schedule');if(r.data?.weeks)official=r.data}catch{}
 const blocked=new Set(base.filter(x=>!String(x.id||'').startsWith('streak-v75-')).map(acceptedCell).filter(Boolean));const clone={...w,schedule:Object.fromEntries(PEOPLE.map(n=>[n,[...(w.schedule[n]||[])]]))};const plan=[];
 for(const n of PEOPLE){if(n==='Marco')continue;let guard=0;while(guard++<12){let streak=priorStreak(n,clone,official),vi=-1,runStart=0;for(let i=0;i<clone.dates.length;i++){if(works(clone.schedule[n][i])){if(streak===0)runStart=i;streak++;if(streak>6){vi=i;break}}else streak=0}if(vi<0)break;const from=Math.max(runStart,vi-5),cand=[];for(let i=from;i<=vi;i++){if(!works(clone.schedule[n][i]))continue;const lab=fmt(clone.dates[i]);if(blocked.has(n+'|'+lab))continue;const sc=candidateScore(clone,n,i);if(Number.isFinite(sc))cand.push({i,sc})}if(!cand.length)break;cand.sort((a,b)=>a.sc-b.sc||a.i-b.i);const i=cand[0].i;clone.schedule[n][i]='RIPOSO';plan.push({employee:n,date:clone.dates[i],shift:'RIPOSO'});blocked.add(n+'|'+fmt(clone.dates[i]))}}
 const next=[...new Map(plan.map(x=>[x.employee+'|'+x.date,x])).values()].sort((a,b)=>(a.date+a.employee).localeCompare(b.date+b.employee));const old=load().sort((a,b)=>(a.date+a.employee).localeCompare(b.date+b.employee));if(JSON.stringify(old)!==JSON.stringify(next)){save(next);const k='tm_streak_reload_v75',c=Number(sessionStorage.getItem(k)||0);if(c<3){sessionStorage.setItem(k,String(c+1));location.replace(location.pathname+'?v=75&streak='+Date.now());return}}sessionStorage.removeItem('tm_streak_reload_v75')}
let timer=null;new MutationObserver(()=>{clearTimeout(timer);timer=setTimeout(optimize,500)}).observe(document.getElementById('app'),{childList:true,subtree:true});window.addEventListener('load',()=>setTimeout(optimize,800));
})();