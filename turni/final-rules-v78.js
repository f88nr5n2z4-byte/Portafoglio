...
function effectiveSalaAt(w,i,t){const dedicated=activeAt(w,i,t,n=>CC.includes(n));const backups=dedicated.length?[]:activeAt(w,i,t,n=>BB.includes(n)).slice(0,1);return activeAt(w,i,t,n=>SS.includes(n)&&!backups.includes(n))}
...
function morningProblems(w,i){const miss=[];if(!RR.some(n=>start2(w.schedule[n]?.[i],'06:30')))miss.push('responsabile alle 06:30');if(!continuousCash(w,i,'07:00','13:30',1))miss.push('1 cassa');if(!continuousEffectiveSala(w,i,'07:00','13:30',2))miss.push('2 sala effettivi');if(!SS.some(n=>start2(w.schedule[n]?.[i],'06:30')))miss.push('1 sala alle 06:30');if(!continuousCount(w,i,'07:00','13:30',()=>true,4))miss.push('4 persone totali');return miss}
function eveningProblems(w,i){const miss=[];if(!continuousCash(w,i,'17:00','20:30',2))miss.push('2 casse');if(!continuousCount(w,i,'17:00','20:30',n=>RR.includes(n),1))miss.push('1 responsabile');if(!continuousEffectiveSala(w,i,'17:00','20:30',1))miss.push('1 sala effettivo');if(!continuousCount(w,i,'17:00','20:30',()=>true,4))miss.push('4 persone totali');return miss}
...
window.validateWeekAdvice=function(w){
 ...
 w.dates.forEach((iso,i)=>{
  const dow=new Date(iso+'T12:00:00').getDay();if(dow===0)return;const label=fmtDate(iso);
  const morning=morningProblems(w,i);if(morning.length)tips.push(`Controlla ${label}: copertura mattina — il conteggio automatico segnala ${morning.join(', ')}`);
  if([1,3,4].includes(dow)&&!continuousCount(w,i,'11:00','17:00',()=>true,5))tips.push(`Controlla ${label}: verifica che ci siano davvero almeno 5 persone dalle 11:00 alle 17:00`);
  const eve=eveningProblems(w,i);if(eve.length)tips.push(`Controlla ${label}: copertura 17:00-20:30 — il conteggio automatico segnala ${eve.join(', ')}`);
  let c2030=0;for(const n of Object.keys(w.schedule))if(at2(w.schedule[n]?.[i],'20:29'))c2030++;
