...
function effectiveSalaAt(w,i,t){const dedicated=activeAt(w,i,t,n=>CC.includes(n));const needBackup=Math.max(0,2-dedicated.length);const backups=activeAt(w,i,t,n=>BB.includes(n)).slice(0,needBackup);return activeAt(w,i,t,n=>SS.includes(n)&&!backups.includes(n))}
...
window.validateWeek=function(w){
 ...
 w.dates.forEach((iso,i)=>{
  ...
  if(!RR.some(n=>start2(w.schedule[n]?.[i],'06:30')))issues.push(`${label}: manca responsabile alle 06:30`);
  if(!continuousCash(w,i,'07:00','13:30',2))issues.push(`${label}: la mattina servono 2 persone abilitate alla cassa`);
  if(!continuousEffectiveSala(w,i,'07:00','13:30',2))issues.push(`${label}: la mattina servono 2 sala effettivi oltre a chi copre la cassa`);
  if(!SS.some(n=>start2(w.schedule[n]?.[i],'06:30')))issues.push(`${label}: manca un sala alle 06:30`);
  ...
  if(!continuousCash(w,i,'17:00','20:30',2))issues.push(`${label}: dalle 17:00 alla chiusura servono 2 persone abilitate alla cassa`);
  if(!continuousCount(w,i,'17:00','20:30',n=>RR.includes(n),1))issues.push(`${label}: dalle 17:00 alla chiusura serve 1 responsabile`);
  if(!continuousEffectiveSala(w,i,'17:00','20:30',1))issues.push(`${label}: dalle 17:00 alla chiusura serve 1 sala effettivo`);
  if(!continuousCount(w,i,'17:00','20:30',()=>true,4))issues.push(`${label}: dalle 17:00 alla chiusura servono almeno 4 persone`);
 });
