(()=>{
 ...
 render=function(){
   ...
   if(actions&&bad.length&&dayIssues(di).length===0){
     const accept=document.getElementById('accept');
     if(accept){accept.disabled=true;accept.textContent='Correggi prima gli altri giorni';accept.style.opacity='.55'}
   }
   if(weeklyOnly.length){
     const note=document.createElement('div');note.className='weekly-issues';note.innerHTML=`<b>Controlli settimanali</b><small>${weeklyOnly.map(esc).join(' · ')}</small>`;
 ...
 .error-day-nav{background:#fff;border:1px solid #dbe5ec;border-radius:17px;padding:11px;margin-bottom:10px}.error-day-label{font-size:11px;font-weight:950;color:#5d7280;text-transform:uppercase;margin-bottom:8px}.error-day-buttons{display:flex;gap:7px;overflow-x:auto;padding-bottom:2px;scrollbar-width:none}.error-day-buttons::-webkit-scrollbar{display:none}.error-day-btn{flex:0 0 auto;border:1px solid #e1c446;background:#fff6c8;color:#5f4e00;border-radius:12px;padding:8px 11px;text-align:left}.error-day-btn b,.error-day-btn span{display:block}.error-day-btn b{color:#003d7c;font-size:12px}.error-day-btn span{font-size:10px;font-weight:900;margin-top:2px}.error-day-btn.active{background:#ffd500;border-color:#d4ad00;box-shadow:0 0 0 2px rgba(255,213,0,.22)}.all-clear{background:#ecf9ef;color:#17602a;border-radius:11px;padding:10px;font-weight:950}.weekly-issues{background:#eef5fa;border:1px solid #cbdde9;border-radius:14px;padding:10px 12px;margin-bottom:10px;color:#003d7c}.weekly-issues b,.weekly-issues small{display:block}.weekly-issues small{margin-top:3px;color:#526b79}
