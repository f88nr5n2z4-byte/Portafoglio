(()=>{
 try{
  const LOCAL_ABSENCES=new Set(['RIPOSO','FERIE','MALATTIA','MATERNITÀ','PERMESSO']);
  let selectedDay=0;
  function firstStart(s){const m=String(s||'').match(/(\d{1,2}):(\d{2})/);return m?(+m[1]*60)+(+m[2]):null}
  function classify(s){if(!s||s==='—'||LOCAL_ABSENCES.has(s))return 'absence';const st=firstStart(s);return st!==null&&st<810?'morning':'afternoon'}
  ...
  function renderDaily(host,w){
    selectedDay=Math.max(0,Math.min((w.dates?.length||1)-1,selectedDay));
  ...
  function mount(){
    ...
    const w=state.data.weeks[state.week];if(!w?.dates?.length||!w.schedule)return;
    ...
    let daily=admin.querySelector('#homeDailySchedule');const weekKey=w.dates[0];
  ...
  let queued=false;
  const obs=new MutationObserver(()=>{if(queued)return;queued=true;requestAnimationFrame(()=>{queued=false;try{mount()}catch(e){console.warn('home daily view',e)}})});obs.observe(document.getElementById('app')||document.documentElement,{subtree:true,childList:true});setTimeout(()=>{try{mount()}catch(e){console.warn('home daily view',e)}},80);
