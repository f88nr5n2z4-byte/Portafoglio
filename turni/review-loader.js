(()=>{
 ...
 function script(src){return new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=src;s.onload=resolve;s.onerror=()=>reject(new Error('Impossibile caricare '+src));document.body.appendChild(s)})}
 async function loadEditor(){for(const src of ['review-change.js?v=88','review-change-fix.js?v=88','review-persist-fix.js?v=88','review-error-days.js?v=88','review-advice-fix.js?v=88'])await script(src)}
 async function boot(){
  ...
  try{
   ...
   if(!dateLabel){app.innerHTML='';await loadEditor();return}
