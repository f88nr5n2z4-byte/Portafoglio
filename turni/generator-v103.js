...
const blankWeek=start=>{const dates=[];for(let i=0;i<7;i++){const d=new Date(start);d.setDate(start.getDate()+i);dates.push(iso(d))}const schedule={};ALL.forEach(n=>schedule[n]=Array(7).fill('RIPOSO'));return{dates,schedule,label:`${dates[0]} / ${dates[6]}`,period:'week',generator:'v103'}};
...
function setup(k){
 ...
 const rr={},cr={},sr={};
 RESP.forEach((n,i)=>rr[n]=((i+k)%3+3)%3);
 CASH.forEach((n,i)=>cr[n]=((i+k)%4+4)%4);
 FLOOR.forEach((n,i)=>sr[n]=((i+k+1)%4+4)%4);
...
function solveDay(activeCash,activeFloor,len,respShifts,marcoWork,splitCount){
 ...
 for(const ca of cc)for(const sa of ss)for(const ms of mopts){
  ...
  const score=[Math.abs(tc-ts),Math.max(...vals)-Math.min(...vals),added,vals.reduce((a,b)=>a+b,0),vals.reduce((a,b)=>a+b*b,0)];
...
function generate(startIso){const m=mondayOf(startIso),phase=phaseFor(m),weeks=[];for(let wi=0;wi<3;wi++){const s=new Date(m);s.setDate(m.getDate()+wi*7);weeks.push(generateWeek(s,phase+wi))}return{weeks,generator:'v103',updated:new Intl.DateTimeFormat('it-IT',{dateStyle:'long',timeStyle:'short'}).format(new Date())}}
...
function validateWeek(w,wi=0){
 ...
 w.dates.forEach((date,d)=>{
  ...
  if(countAt(w,d,'10:00')<5)add(out,'copertura',wi,date,null,`${lab}: alle 10:00 servono almeno 5 persone totali`);
  if(countAt(w,d,'13:30')<5)add(out,'copertura',wi,date,null,`${lab}: alle 13:30 servono almeno 5 persone totali`);
  if(countAt(w,d,'16:30')<5)add(out,'copertura',wi,date,null,`${lab}: alle 16:30 servono almeno 5 persone totali`);
