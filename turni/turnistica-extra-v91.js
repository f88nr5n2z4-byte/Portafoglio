...
function weekExtra(w,wi){const out=[];
 ...
 const sun=(w.dates||[]).findIndex(C.isSunday);
 if(sun>=0){
  const fullTime=[...C.RESPONSABILI,...C.CASSA.filter(n=>n!=='Giada'),...C.SALA.filter(n=>n!=='Marco')];
  fullTime.forEach(n=>{
   if(!C.works(w.schedule?.[n]?.[sun]))return;
   if(Math.abs(C.hours(w.schedule[n][sun])-5)>.001)out.push(add('ore',wi,w.dates[sun],n,`${n}: la domenica deve fare 5h`));
   for(let d=0;d<6;d++){
    const s=w.schedule?.[n]?.[d];if(!C.works(s))continue;
    const h=C.hours(s);if(Math.abs(h-7)>.001)out.push(add('ore',wi,w.dates[d],n,`${C.dateLabel(w.dates[d])}: ${n} lavora domenica, quindi il turno feriale deve essere di 7h (ora ${h}h)`));
   }
  });
  if(C.works(w.schedule?.Giada?.[sun])&&Math.abs(C.hours(w.schedule.Giada[sun])-5)>.001)out.push(add('ore',wi,w.dates[sun],'Giada','Giada: la domenica deve fare 5h'));
 }
 return out}
