...
new_view = '''function absencesView(){
  ...
  return shell(`<section class="form-card"><h3>Aggiungi assenza</h3><div class="form-grid"><label><span>Dipendente</span><select id="absPerson">${PEOPLE.map(n=>`<option>${esc(n)}</option>`).join('')}</select></label><label><span>Tipo</span><select id="absType"><option>FERIE</option><option>PERMESSO</option><option>RIPOSO</option><option>CONGEDO</option><option>MALATTIA</option></select></label><label><span>Dal</span><input id="absFrom" type="date"></label><label><span>Al</span><input id="absTo" type="date"></label></div><button class="btn primary" id="addAbs">Aggiungi</button><div class="info-box">ⓘ Un RIPOSO inserito o accettato come richiesta vale come riposo settimanale: il generatore non deve aggiungerne un secondo.</div></section><section class="list-card"><h3>Ferie e assenze inserite</h3>${rows.length?`<div class="absence-list">${rows.map(a=>`<div><span class="absence-icon">${String(a.absence_type).toUpperCase()==='RIPOSO'?'▣':String(a.absence_type).toUpperCase()==='CONGEDO'?'◫':'☂'}</span><div><b>${esc(a.employee_name)}</b><small>${esc(a.absence_type)} · ${esc(a.date_from)} → ${esc(a.date_to||a.date_from)}${a.fromRequest?' · richiesta accettata':''}</small></div>${a.fromRequest?'':`<button data-delabs="${a.id}">×</button>`}</div>`).join('')}</div>`:'<div class="empty-mini">Nessuna assenza inserita.</div>'}</section>`)
}'''
s = s[:start] + new_view + s[end:]
p.write_text(s)

ip = Path('turni-v2/index.html')
x = ip.read_text().replace('current-20260913-4','current-20260913-5').replace('current-20260913-3','current-20260913-5')
ip.write_text(x)

sw = Path('turni-v2/sw.js')
x = sw.read_text().replace('turni-current-20260913-v4','turni-current-20260913-v5').replace('turni-current-20260913-v3','turni-current-20260913-v5')
sw.write_text(x)
